...
class Keycloak(object):
    ...
    def authenticate_user(self):
        ...
        Users are redirected to this URI with required parameters, where Keycloak
        authenticates the user.
        In response, keycloak adds authorization code, which is used to fetch tokens

        """
        params = {'client_id': client_id, 'redirect_uri': '{}/login'.format(redirect_uri),
                  'response_type': 'code'}
    ...
    def get_access_token(self, auth_code):
        ...
        token_endpoint_params = {'grant_type': 'authorization_code', 'code': auth_code,
        ...
        tokens = requests.post('{0}/auth/realms/{1}/protocol/openid-connect/token'.format(base_uri, realm),
                               data=token_endpoint_params)
        token_dict = json.loads(tokens.text)
        access_token = token_dict['access_token']
        refresh_token = token_dict['refresh_token']
        user_endpoint_params = {'access_token': access_token}
        # Make request to keycloak's user_info endpoint in order to fetch the
        # username. We need to set this in cookie
        user_info = requests.post('{0}/auth/realms/{1}/protocol/openid-connect/userinfo'.format(base_uri, realm),
                                  data=user_endpoint_params)
    ...
    def refresh_access_token(self, refresh_token):
        ...
        If the access token is expired and refresh token is valid, the token is
        validated and new access token is fetched.
        ...
        refresh_token_endpoint_params = {'grant_type': 'refresh_token', 'refresh_token': refresh_token,
                                         'client_id': client_id, 'client_secret': client_secret}
    ...
    def verify_signature(self, access_token, refresh_token):
        ...
        If the user comes with access token, this verifies the signature.
        If verification results in error, refresh token is validated.
        ...
        user_info = requests.post('{0}/auth/realms/{1}/protocol/openid-connect/token/introspect'.format(base_uri, realm),
